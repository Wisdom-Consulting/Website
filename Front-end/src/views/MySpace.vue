...
<template>
  ...
  <div class="container mx-auto px-4 sm:px-6 lg:px-8 mt-10">
    ...
    >
      <swiper-slide v-for="quiz in mySpaceStore.quizzes" class="w-fit">
        <div class="bg-white shadow-lg rounded-lg w-full h-fit p-6 flex flex-col gap-2 self-center">
          <div class="flex gap-2">
            <img v-bind:src="'./src/assets/eye.png'" class="w-5" alt="">
            <span class="text-[#003333] text-sm"> 5 min</span>
          </div>
          <div class="flex flex-row gap-2 text-center justify-center font-bold h-10 mb-1">
            <h1 class="text-[#00333] text-md">{{quiz.title}}</h1>
          </div>
          <div class="flex flex-col gap-4 justify-center text-center">
            <h3 class="font-medium text-[15px] text-wrap text-[#003333] h-20 mb-1">{{quiz.body.slice(0,100)}}</h3>
          </div>
<!--          Pass quiz button-->
